...
User_ApoMsg_Eval (int Ch, char *Msg, int len, int who)
{
    ...
    const int channel = 2;
        int i;
         ...
         /* get list of servers */
         ApocQueryServers(2000 /* ms */, NULL);
         do {
         if (ApocWaitIO(100 /* ms */) == 1)
         ApocPoll();
         } while (!ApocQueryDone());
         ...
         LogUsage("Server found\n");
         LogUsage("channelmask %lo\n", ApocGetAppMask (sid));
         break;
         }
         }
         if (sid == NULL)
         ...
         /* connect to server */
         ApocConnect(sid, 1<<channel /*mask*/);
         do {
         if (ApocWaitIO(100 /* ms */) == 1)
         ApocPoll();
         } while (ApocGetStatus(sid, NULL) == ApoConnPending);
         if (!(ApocGetStatus(sid, NULL) & ApoConnUp)){
        	 LogUsage("could not connect --->");
        	 LogUsage("Connection status: %s\n",ApoStrStatus(ApocGetStatus(sid, NULL)));
        	 return 0;
         }

         int status = ApocGetStatus(sid, NULL);

         subs[0].Name = "Sine";
         ApocSubscribe(sid, 1, subs, 100, 1, 10, 0);

         ApocPoll();

         LogUsage("Connection status: %s\n",ApoStrStatus(status));
         LogUsage("Connection status: %i\n",status);
        /* connect failed */
